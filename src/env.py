...
class Env(object):
    ...
    def __init__(self, baseline, state_size=3):
        ...
        delta = np.random.multivariate_normal(
            ...
            cov=np.identity(beta.shape[0]) * sigma_beta,
            size=self.nb_users)
    ...
    def step(self, state, action):
        ...
        xi = np.random.multivariate_normal(
            ...
            cov=np.identity(self.state_size) * sigma_s)
        rho = np.random.normal(0, sigma_r)
        s_new = np.zeros_like(state)
        s_new[:, 0] = np.tanh(beta[:, 0] * state[:, 0] + xi[0])
        s_new[:, 1] = np.tanh(beta[:, 1] * state[:, 1]
                              + beta[:, 2] * action
                              + xi[1])
        s_new[:, 2] = np.tanh(beta[:, 3] * state[:, 2]
                              + beta[:, 4] * state[:, 2] * action
                              + beta[:, 5] * action
                              + xi[2])
        for i in range(3, self.state_size):
            s_new[i] = np.tanh(beta[:, 6] * state[:, i] + xi[i])
        reward = beta[:, 7] * (b + action * (beta[:, 8]
                               + beta[:, 9] * state[:, 0]
                               + beta[:, 10] * state[:, 1])
                               + beta[:, 11] * state[:, 0]
                               - beta[:, 12] * state[:, 2]
    ...
    def reset(self):
        SIGMA = np.identity(self.state_size)
